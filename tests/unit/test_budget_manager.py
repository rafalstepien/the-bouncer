...
@pytest.fixture
def budget_service():
    return DefaultBudgetManagerService(
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            100, PIPELINE_1, 100, {PIPELINE_1: 100, PIPELINE_2: 0},
            id="Update budget adds tokens correctly"
        ),
        pytest.param(
            0, PIPELINE_2, 0, {PIPELINE_1: 0, PIPELINE_2: 0},
            id="Update budget handles zero tokens correctly"
        )
    ]
)
def test_update_budget(budget_service: DefaultBudgetManagerService, tokens_to_add: int, pipeline: SourcePipeline, expected_global: int, expected_pipeline: dict[str, int]):
...
def test_refill_logic_resets_after_interval(budget_service: DefaultBudgetManagerService):
...
def test_refill_threshold_boundaries(budget_service: DefaultBudgetManagerService, seconds_passed: int, should_refill: bool):
...
def test_initial_call_sets_last_updated(budget_service: DefaultBudgetManagerService):
